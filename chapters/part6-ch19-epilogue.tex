...
\begin{tikzpicture}[
  ...
  \node[font=\small\bfseries\color{dogmateal}] at (-2.5, 2.5) {Biology:};
  \node[stage, fill=blue!12] (dna) at (0, 2.5) {DNA\\Genome};
  \node[stage, fill=green!12] (reg) at (3.2, 2.5) {Gene\\Regulation};
  \node[stage, fill=yellow!12] (txn) at (6.4, 2.5) {Transcription\\(mRNA)};
  \node[stage, fill=orange!12] (tln) at (9.6, 2.5) {Translation\\\& Folding};
  \node[stage, fill=red!12] (phn) at (12.8, 2.5) {Functional\\Phenotype};
  ...
  \draw[arrow, dogmateal, bend left=22] (phn.north) to node[above, font=\scriptsize, align=center] {Mutation \&\\Selection} (dna.north);
  \draw[arrow, dogmared, bend right=22] (real.south) to node[below, font=\scriptsize, align=center] {Evolutionary\\Synthesis} (gen.south);
\end{tikzpicture}
