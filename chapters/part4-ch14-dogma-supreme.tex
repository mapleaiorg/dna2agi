...
\centering
\begin{tikzpicture}[
    stage/.style={draw, rounded corners=4pt, minimum width=4.2cm, minimum height=1.1cm, font=\small, align=center},
  ...
  \node[stage, fill=purple!12, minimum width=3cm] (epi) at (6.5, -4.4) {Epigenetic Memory\\(256 slots)};
  \draw[<->, thick, purple!60] (s4.east) ++(2.0,0.5) -- (epi.west) node[midway, above, font=\scriptsize] {read/write};

  % TetraMemory (side, bottom)
  \node[stage, fill=purple!12, minimum width=3cm] (mem) at (6.5, -6.4) {TetraMemory\\(multi-scale)};
  \draw[<->, thick, purple!60] (s4.east) ++(2.0,-0.5) -- (mem.west) node[midway, above, font=\scriptsize] {read/write};
...
\begin{tikzpicture}[
  ...
  \node[app, fill=blue!8] (nlp) at (-6.5, -6.5) {NLP\\(LM, summarization)};
  \node[app, fill=green!10] (chat) at (-3.5, -6.5) {Conversational AI\\(ChatSession)};
  \node[app, fill=purple!8] (multi) at (-0.5, -6.5) {Multimodal\\(3D, image, audio)};
  \node[app, fill=red!8] (genomic) at (2.5, -6.5) {Genomics\\(sequence analysis)};
  \node[app, fill=orange!10] (science) at (5.5, -6.5) {Scientific\\(drug design)};
  \node[app, fill=dogmawarm] (code) at (8.5, -6.5) {Code\\(completion)};
  ...
  \node[font=\small\bfseries, dogmateal] at (-5.5, -6.5) {Applications};
\end{tikzpicture}
