...
% Level 1
\node[levelone, below left=1.5cm and 2.5cm of root] (outer) {Outer Alignment\\(Objective Specification)};
\node[levelone, below right=1.5cm and 2.5cm of root] (inner) {Inner Alignment\\(Learned Objectives)};

% Level 2 - Outer
\node[leveltwo, below left=1.2cm and 0.5cm of outer] (goodhart) {Goodhart's Law\\(Proxy Gaming)};
\node[leveltwo, below right=1.2cm and 0.5cm of outer] (reward) {Reward\\Hacking};

% Level 2 - Inner
\node[leveltwo, below left=1.2cm and 0.5cm of inner] (mesa) {Mesa-Optimization\\(Emergent Goals)};
\node[leveltwo, below right=1.2cm and 0.5cm of inner] (deceptive) {Deceptive\\Alignment};

% Mitigations row
\node[rootbox, below=4.5cm of root, text width=5cm, draw=dogmared, fill=dogmared!8] (mitigate)
  {Mitigation Strategies};

\node[leveltwo, below left=1.2cm and 2.5cm of mitigate, text width=2.8cm, draw=dogmared!60, fill=dogmared!5]
...
\node[leveltwo, below right=1.2cm and 2.5cm of mitigate, text width=2.8cm, draw=dogmared!60, fill=dogmared!5]
...
\begin{tikzpicture}[
  ...
  biobox/.style={rectangle, rounded corners=4pt, draw=dogmagold, fill=dogmawarm,
    text width=3.5cm, minimum height=0.8cm, align=center, font=\small\itshape},
  arrow/.style={-{Stealth[length=5pt]}, thick, dogmablue},
...
\node[stagebox, below=1.5cm of genome] (regulate) {Regulation\\Stage};
\node[stagebox, below=1.5cm of regulate] (express) {Expression\\Stage};
\node[stagebox, below=1.5cm of express] (evolve) {Evolutionary\\Selection};

% Safety mechanisms (right side)
\node[mechbox, right=2.5cm of genome] (audit) {Genome Audit\\Protocol};
\node[mechbox, right=2.5cm of regulate] (constit) {Constitutive\\Repression};
\node[mechbox, right=2.5cm of express] (filter) {Output Filtering\\and Monitoring};
\node[mechbox, right=2.5cm of evolve] (pareto) {Safety-Augmented\\Pareto Selection};

% Biological analogues (far right)
\node[biobox, right=1.5cm of audit] {DNA repair\\mechanisms};
\node[biobox, right=1.5cm of constit] {Epigenetic\\silencing};
\node[biobox, right=1.5cm of filter] {Immune\\surveillance};
\node[biobox, right=1.5cm of pareto] {Multi-objective\\natural selection};
...
% Bio arrows
\draw[bioarrow] (audit.east) -- ++(1.5,0);
\draw[bioarrow] (constit.east) -- ++(1.5,0);
\draw[bioarrow] (filter.east) -- ++(1.5,0);
\draw[bioarrow] (pareto.east) -- ++(1.5,0);
...
\node[above right=0.3cm and 1.5cm of audit, font=\bfseries\small, color=dogmagold] {Biological Analogue};
