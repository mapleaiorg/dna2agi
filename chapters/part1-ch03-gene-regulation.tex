...
\begin{tikzpicture}[
  ...
  % RNA Polymerase
  \node[protein, fill=dogmawarm, minimum width=1.8cm] (rnap) at (4.5,1.5) {RNA Pol II};
  \draw[-{Latex[length=2mm]}, dashed, gray] (rnap) -- (4.2,0.2);

  % Transcription factors
  \node[protein, minimum width=1.2cm] (tf1) at (2.7,1.5) {TF};
  \draw[-{Latex[length=2mm]}, dashed, gray] (tf1) -- (2.7,0.2);

  % Arrow showing transcription direction
  \draw[-{Latex[length=3mm]}, very thick, dogmateal] (5,0.5) -- (8,0.5)
...
\begin{tikzpicture}[
  ...
  \draw[-{Latex[length=2mm]}, dashed, dogmateal] (act.east) -- (pol1.west)
    node[midway, above, font=\tiny] {Recruits};
  ...
  % RNA Pol blocked
  \node[protein, fill=gray!30] (pol2) at (2.5, -2.8) {RNA Pol};
...
\begin{tikzpicture}[
  ...
  \node[font=\tiny, anchor=north] at (11.5, 2.1) {
    \begin{tabular}{cc|c}
    Glu & Lac & Express \\
    \hline
    0 & 0 & 0 \\
...
\begin{tikzpicture}[
  ...
  \node[font=\bfseries] at (-2, 2) {Electronic:};
  ...
  \draw[signal] (n2.east) -- (n3.east |- n2.east);
  \draw[signal] (n3.east) -- ++(0.8,0) |- ++(0,1.2) -| ($(n1.west)+(-0.8,0)$) -- (n1.west);

  % === Biological repressilator (bottom) ===
  \node[font=\bfseries] at (-2, -1) {Biological:};

  \node[gene] (g1) at (0, -1) {LacI};
  \node[gene] (g2) at (3, -1) {TetR};
  \node[gene] (g3) at (6, -1) {$\lambda$CI};

  \draw[signal, dogmared, dashed] (g1) -- (g2) node[midway, above, font=\tiny] {represses};
  \draw[signal, dogmared, dashed] (g2) -- (g3) node[midway, above, font=\tiny] {represses};
  \draw[signal, dogmared, dashed] (g3.east) -- ++(0.8,0) |- ++(0,-1.2) -| ($(g1.west)+(-0.8,0)$) -- (g1.west);
  \node[font=\tiny] at (3, -2.8) {represses};

  % Equivalence
  \draw[{Latex[length=2mm]}-{Latex[length=2mm]}, thick, gray, dashed] (3, 0.7) -- (3, -0.2)
    node[midway, right, font=\tiny, text=gray] {equivalent};
\end{tikzpicture}
...
\begin{tikzpicture}[
  ...
  % State 1
  \node[font=\bfseries] at (0, 2.2) {State 1: Gene A dominant};
  ...
  \draw[signal, gray!40, dashed] (b1.south) to[bend right=40] node[below, font=\tiny, gray] {too weak} (a1.south);

  % State 2
  \node[font=\bfseries] at (7, 2.2) {State 2: Gene B dominant};
  ...
  \draw[signal, dogmared, dashed, thick] (b2.south) to[bend right=40] node[below, font=\tiny] {represses} (a2.south);

  % Flip arrow
  \draw[{Latex[length=3mm]}-{Latex[length=3mm]}, very thick, dogmagold] (3.3, 1) -- (4.7, 1)
    node[midway, above, font=\tiny\bfseries] {Transient};
  \node[font=\tiny\bfseries, dogmagold] at (4, 0.5) {signal};

  % Digital analogy
  \node[draw, dashed, gray, rounded corners, inner sep=5pt, font=\tiny, text=gray] at (4, -1.2)
...
\begin{tikzpicture}[
  ...
  \node[font=\tiny, dogmagold, below] at (6, 0.5) {Attractor 2 (fixed point)};
  \node[font=\tiny, dogmagold, below] at (6, 0.1) {``Cell type B''};
...
\begin{tikzpicture}[
  ...
  % === Negative feedback ===
  \node[font=\footnotesize\bfseries] at (1.5, 2) {Negative Feedback};
  \node[box] (a1) at (0, 0.8) {$A$};
  \node[box] (b1) at (3, 0.8) {$B$};
  \draw[posarr] (a1) -- (b1) node[midway, above, font=\tiny] {activates};
  \draw[negarr] (b1.south) to[bend right=35] node[below, font=\tiny] {inhibits} (a1.south);
  \node[font=\tiny, gray] at (1.5, -0.4) {Effect: damping, stability};

  % === Positive feedback ===
  \node[font=\footnotesize\bfseries] at (7.5, 2) {Positive Feedback};
  \node[box] (a2) at (6, 0.8) {$A$};
  \node[box] (b2) at (9, 0.8) {$B$};
  \draw[posarr] (a2) -- (b2) node[midway, above, font=\tiny] {activates};
  \draw[posarr] (b2.south) to[bend right=35] node[below, font=\tiny] {activates} (a2.south);
  \node[font=\tiny, gray] at (7.5, -0.4) {Effect: bistability, switch};

  % === Combined ===
  \node[font=\footnotesize\bfseries] at (13, 2) {Combined};
  \node[box] (a3) at (12, 0.8) {$A$};
  \node[box] (b3) at (14.5, 0.8) {$B$};
  \draw[posarr] (a3) -- (b3) node[midway, above, font=\tiny] {activates};
  \draw[posarr] (b3.north) to[bend left=35] node[above, font=\tiny] {$+$} (a3.north);
  \draw[negarr] (b3.south) to[bend right=35] node[below, font=\tiny] {$-$} (a3.south);
  \node[font=\tiny, gray] at (13.25, -0.4) {Effect: oscillation, adaptation};
\end{tikzpicture}
...
\begin{tikzpicture}[
  ...
  % TF binds
  \node[draw, rounded corners, fill=dogmamint, font=\tiny] (tf) at (5, 1.5) {TF binds};
  \draw[-{Latex[length=1.5mm]}, dogmateal] (tf) -- (5, 0.6);

  \node[font=\tiny, dogmateal] at (11, 0.5) {Gene ON};
  \draw[-{Latex[length=2mm]}, thick, dogmateal] (10.2, 0.42) -- (10.8, 0.42);

  % === Methylated (silenced) ===
  \node[font=\footnotesize\bfseries] at (-1, -1.2) {Methylated (Silenced):};
  \draw[dna, gray] (2, -1.7) -- (10, -1.7);
  \draw[dna, gray] (2, -1.85) -- (10, -1.85);

  % CpG sites with methylation
  \foreach \x in {3, 5, 7} {
    \node[methyl] at (\x, -1.35) {M};
    \node[font=\tiny, gray] at (\x, -1.15) {CpG};
  }

  % TF blocked
  \node[draw, rounded corners, fill=gray!20, font=\tiny, text=gray] (tf2) at (5, -0.5) {TF blocked};
  \draw[-{Latex[length=1.5mm]}, gray, dashed] (tf2) -- (5, -1.2);
  \node[font=\Large, dogmared] at (5, -0.9) {$\times$};

  \node[font=\tiny, dogmared] at (11, -1.7) {Gene OFF};
  \draw[thick, dogmared] (10.3, -1.6) -- (10.7, -1.9);
  \draw[thick, dogmared] (10.3, -1.9) -- (10.7, -1.6);
\end{tikzpicture}
