...
\begin{tikzpicture}[
  ...
  % Legend
  \node[font=\scriptsize, dogmablue] at (7.5, 4.2) {Low attention};
  \draw[attn] (6.5, 4.2) -- (7.0, 4.2);
  \node[font=\scriptsize, dogmared] at (7.5, 3.7) {High attention};
...
\begin{tikzpicture}[
  ...
  \draw[skip] (input.east) -- ++(1.8,0) |- (add1.east);
  \draw[skip] (add1.east) -- ++(2.1,0) |- (add2.east);

  % Labels for residual
  \node[font=\scriptsize, gray] at (2.6, 1.6) {residual};
  \node[font=\scriptsize, gray] at (2.9, 4.8) {residual};

  % Annotations
  \node[annot, anchor=west] at (3.2, 2.2) {$h$ parallel heads\\each $d_k = d/h$};
  \node[annot, anchor=west] at (3.2, 5.4) {$\text{ReLU}(\mathbf{x}\mathbf{W}_1\!+\!\mathbf{b}_1)\mathbf{W}_2\!+\!\mathbf{b}_2$\\inner dim $= 4d$};
...
\centering
\begin{tikzpicture}[
  block/.style={draw, rounded corners=4pt, minimum width=2.6cm, minimum height=0.7cm, font=\small\sffamily, align=center},
...
% Column headers
\node[font=\bfseries\sffamily, text=dogmablue] at (0, 4.5) {Transformer};
\node[font=\bfseries\sffamily, text=dogmagray] at (3.6, 4.5) {Analogy};
\node[font=\bfseries\sffamily, text=dogmateal] at (7.2, 4.5) {DOGMA};

% Transformer column
\node[block, fill=dogmalight] (tpe) at (0, 3.6) {Positional\\Encoding};
\node[block, fill=dogmalight] (temb) at (0, 2.5) {Token\\Embedding};
\node[block, fill=dogmalight] (tatt) at (0, 1.4) {Self-Attention};
\node[block, fill=dogmalight] (tffn) at (0, 0.3) {Feed-Forward\\Network};
\node[block, fill=dogmalight] (tout) at (0, -0.8) {Linear +\\Softmax};

% DOGMA column
\node[block, fill=dogmamint] (dge) at (7.2, 3.6) {Genomic\\Embedding};
\node[block, fill=dogmamint] (dreg) at (7.2, 2.5) {Typed Region\\Encoding};
\node[block, fill=dogmamint] (dsa) at (7.2, 1.4) {Strand\\Attention};
\node[block, fill=dogmamint] (ddna) at (7.2, 0.3) {DNA Computing\\Block};
\node[block, fill=dogmamint] (drea) at (7.2, -0.8) {Realization\\Stage};
