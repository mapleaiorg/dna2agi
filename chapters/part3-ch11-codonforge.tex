...
\begin{tikzpicture}[
  ...
  \draw[arrow, color=dogmablue] (mrna) -- node[above, font=\scriptsize] {read codon} (ribosome);
  \draw[arrow, color=dogmateal] (ribosome) -- node[above, font=\scriptsize] {match anticodon} (trna);
  \draw[arrow, color=dogmagold] (trna) -- node[above, font=\scriptsize] {add amino acid} (protein);

  % Analogies below
  \node[below=0.6cm of mrna, font=\scriptsize\itshape, text=dogmagray] {(CodonForge program)};
  \node[below=0.6cm of ribosome, font=\scriptsize\itshape, text=dogmagray] {(Interpreter)};
  \node[below=0.6cm of trna, font=\scriptsize\itshape, text=dogmagray] {(Opcode dispatch)};
  \node[below=0.6cm of protein, font=\scriptsize\itshape, text=dogmagray] {(Output)};
\end{tikzpicture}
...
\begin{tikzpicture}[
  ...
  \draw[arrow] (codon) -- node[right, font=\scriptsize] {SDANS compile} (supreme);
  \draw[arrow] (tera) -- node[above, font=\scriptsize] {eval} (supreme);
  \draw[arrow] (tetra) -- node[above, font=\scriptsize] {data} (supreme);
  \draw[arrow, dashed] (supreme) -- node[right, font=\scriptsize, text=dogmagray] {feedback} (codon);
\end{tikzpicture}
