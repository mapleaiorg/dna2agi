...
\begin{example}[Worked Example: Processing a Sequence Through a 2-State SSM]
Let us trace a concrete 4-element sequence $u = [1.0, 0.5, 0.8, 0.2]$ through a minimal SSM with state dimension $n=2$. We use:
...
\textbf{Step 1} ($u_1 = 1.0$, start from $x_0 = [0, 0]^\top$):
\[
...
\begin{example}[Worked Example: Selective Scan with Actual Numbers]
Process the same sequence $u = [1.0, 0.5, 0.8, 0.2]$ through a \emph{selective} Mamba layer ($n=2$). Unlike the fixed SSM above, here $\mathbf{B}_k$ and $\Delta_k$ are input-dependent:
...
\textbf{Step 1} ($u_1 = 1.0$): $\Delta_1 = \text{softplus}(1.0 - 0.5) = \ln(1 + e^{0.5}) \approx 0.974$.
\[
...
\textbf{Step 2} ($u_2 = 0.5$): $\Delta_2 = \text{softplus}(0.5 - 0.5) = \ln 2 \approx 0.693$.
\[
...
\subsection{Parameter Allocation: DOGMA vs.\ Transformer}

To understand DOGMA's scaling at a concrete level, let us compare exactly where 224.7M parameters are allocated in DOGMA-Supreme versus a standard Transformer of equal size.

\begin{table}[H]
\centering
\caption{Parameter allocation comparison: DOGMA-Supreme (224.7M) vs.\ Transformer (224M). Both use $d=512$, $T=512$.}
\label{tab:param-allocation}
\renewcommand{\arraystretch}{1.15}
\begin{tabularx}{0.95\textwidth}{L{4cm} C{2.5cm} C{2.5cm} Y}
\toprule
\textbf{Component} & \textbf{DOGMA} & \textbf{Transformer} & \textbf{Notes} \\
\midrule
...
\midrule
\textbf{Total} & \textbf{224.7M} & \textbf{224M} & \\
\bottomrule
\end{tabularx}
\end{table}

\begin{keyinsight}[Where the Parameters Go]
The fundamental difference is \emph{diversity vs.\ uniformity}. A Transformer distributes parameters equally across identical layers, each with the same attention + FFN structure. DOGMA distributes parameters across \emph{functionally specialized} components. This means:
\begin{itemize}[leftmargin=1.5em]
  \item DOGMA's 4-path DNA Computing Block uses 39\% of parameters but provides three independent Turing-completeness proofs---a Transformer's FFN is a universal approximator but not a structured computer.
  \item DOGMA's tiny embedding layer (0.13M vs.\ 16.4M) reflects byte-level tokenization: 257 tokens vs.\ 32,000 BPE tokens. This is more parameter-efficient but shifts the burden of ``understanding'' tokens to the DNA computing layers.
  \item DOGMA spends 15\% of parameters on TetraMemory (local structure) and 20\% on Strand Attention (global structure), providing a natural local/global split that Transformers achieve implicitly through attention head specialization.
\end{itemize}
\end{keyinsight}

\subsection{Per-Block FLOP Analysis}
...
\begin{center}
\renewcommand{\arraystretch}{1.15}
\begin{tabularx}{0.95\textwidth}{L{4cm} C{3cm} Y}
\toprule
\textbf{Stage} & \textbf{FLOPs per block} & \textbf{Computation} \\
\midrule
...
\midrule
\textbf{Total per block} & $\sim$4.7B & \\
\textbf{16 blocks} & $\sim$75B & Comparable to 12-layer Transformer ($\sim$72B) \\
\bottomrule
\end{tabularx}
\end{center}

The total per-forward-pass cost is comparable to a same-sized Transformer, but the \emph{distribution} of compute is fundamentally different: DOGMA spends 45\% of FLOPs in the DNA Computing Block (structured computation) while a Transformer spends 67\% in FFN layers (unstructured matrix multiplications).
